...
def retrieve_episodes(dataset):
    ...
    for traj in tqdm(dataset, desc="Retrieve episodes", total=len(dataset)):
        ...
        for step in traj["steps"]:
            ...
            hand_image_right = step["observation"]["hand_image_right"].numpy()
            action = step["action"].numpy()
            steps.append(
                {
                    "observation": {
                        "image": image,
                        "hand_image_right": hand_image_right,
                        "state": state,
                    },
...
def process_episode(
    ...
    for episode_index, episode in tqdm(
        ...
        for step in episode:
            ...
            # clean the flag for multiple same category objects
            step["language_instruction"] = remove_brackets_and_numbers(text_instruction)
...
def get_mm_utils_object(text_instruction, text2mmins, mmins2utils, utils_folder):
    ...
    if text_instruction in text2mmins:
        ...
        for item in mm_placeholders:
            random_idx = np.random.randint(1, 4)
            item = f"{item} {random_idx}"
...
def get_mm_utils_ocr(text_instruction, text2mmins, mmins2utils, utils_folder):
    ...
    if text_instruction in text2mmins:
        mm_instruction = text2mmins[text_instruction]
        mm_utils = []
        if mm_instruction in mmins2utils:
            mm_utils.append(utils_folder / (mmins2utils[mm_instruction] + f"_style{style_id}.png"))
        else:
            raise ValueError(f"'{mm_instruction}' not found in map file")
    else:
        raise ValueError(
            f"'{text_instruction}' not found in resources, check consistency of resources"
        )
...
@dataclass
class DataConfig:
    dataset_path: str = "/liujinxin/code/rlds_dataset_builder/tensorflow_datasets/"
    debug_mode: bool = True

    object_text2mmins: str = (
        "/liujinxin/zhaowei/CogACT/vla/piper_res/piper_object.yaml"
    )
    object_mmins2utils: str = (
        "/liujinxin/zhaowei/CogACT/vla/piper_res/piper_object_map.yaml"
    )
    object_utils: str = "/liujinxin/zhaowei/CogACT/vla/piper_res/utils/piper_object"

    ocr_text2mmins: str = "/liujinxin/zhaowei/CogACT/vla/piper_res/piper_ocr.yaml"
    ocr_mmins2utils: str = (
        "/liujinxin/zhaowei/CogACT/vla/piper_res/piper_ocr_map.yaml"
    )
    ocr_utils: str = "/liujinxin/zhaowei/CogACT/vla/piper_res/utils/piper_ocr"
    # target directory for processed episodes
    target_path: str = (
        "/liujinxin/zhaowei/rlds_dataset_builder/npy_files/processed_oe_piper_table_mix"
    )
...
def main(cfg: DataConfig):
    ...
    raw_dataset_builder = tfds.builder("piper_table_mix", data_dir=dataset_path)
