...
class VLAServer:
    ...
    def compose_input(
        self, img_static, img_gripper, instruction, robot_obs, obj_image=None,
        goal_img_static=None, video_static=None, ins_image=None, debug=False
    ):
        if debug:
            img_static.save("imgs_debug/eval_scene.png")
            img_gripper.save("imgs_debug/eval_gripper.png")

        img_all = {"scene": img_static, "left": img_gripper}
        ...
        if self.instruction_type == "text":
            ...
            raise NotImplementedError(f"Instruction type {self.instruction_type} not implemented!")
        return img_all, img_utils, oe_lang

    def generate_action(self, img_all, img_utils, oe_lang, robot_obs):
    ...
    def predict():
        if request.method == "POST":
            ...
            assert instruction_type in [
                "text", "mmins", "goal_image", "imitation_video", "ins_image"
            ...
            # compose the input
            img_all, img_utils, oe_lang = vla_robot.compose_input(img_static, img_gripper, instruction, robot_obs, 
                                                obj_image=obj_img_data, goal_img_static=goal_img_static, 
                                                video_static=video_static_list, ins_image=ins_image, 
                                                debug=args.debug)
