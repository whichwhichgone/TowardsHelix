...
def make_oxe_dataset_kwargs(
    ...
    if dataset_kwargs["action_encoding"] not in [ActionEncoding.JOINT_BIMANUAL_AGIBOT]:
        raise ValueError(f"Cannot load `{dataset_name}`; only JOINT_BIMANUAL_AGIBOT actions supported!")

    # [Contract] For JOINT_BIMANUAL_AGIBOT actions, the 8th and 16th action dimension (gripper) are absolute!
    # Normalize all action dimensions *except* the gripper
    if dataset_kwargs["action_encoding"] is ActionEncoding.JOINT_BIMANUAL_AGIBOT:
        dataset_kwargs["absolute_action_mask"] = 2 * ([False] * 7 + [True])
        dataset_kwargs["action_normalization_mask"] = 2 * ([True] * 7 + [False])
