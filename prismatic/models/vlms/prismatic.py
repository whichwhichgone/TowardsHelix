...
class PrismaticVLM(VLM):
    ...
    def freeze_backbones(self, stage: str) -> None:
        ...
        # TODO: zzf update
        elif stage == "all-frozen":
            # Freeze all components: vision_backbone, llm_backbone, and projector
            self.vision_backbone.requires_grad_(False)
            self.llm_backbone.requires_grad_(False)
            self.projector.requires_grad_(False)
            ...
            # Explicitly Log Frozen Components
            # fmt: off
            overwatch.info(f"[Frozen] 🥶 =>> Vision Backbone `{self.vision_backbone.identifier}`", ctx_level=1)
            overwatch.info(f"[Frozen] 🥶 =>> LLM Backbone `{self.llm_backbone.identifier}`", ctx_level=1)
            overwatch.info(f"[Frozen] 🥶 =>> Projector `{self.arch_specifier}`", ctx_level=1)
    ...
    def forward(
        self,
        input_ids: Optional[torch.LongTensor] = None,                   # 文本数据在经过tokenizer后的输出
        attention_mask: Optional[torch.Tensor] = None,                  # 注意力掩码，标记哪些位置是填充的
        pixel_values: Optional[torch.FloatTensor] = None,              
        labels: Optional[torch.LongTensor] = None,                      # 文本数据的标签
        ...
        output_hidden_states: Optional[bool] = None,                    # 是否返回每一层隐藏状态
        return_dict: Optional[bool] = None,                             # 是否返回字典格式的输出    
        multimodal_indices: Optional[torch.LongTensor] = None,          # 多模态索引，哪些位置要参与多模态，None表示都为，[0,1,2,3..]
    ) -> CausalLMOutputWithPast:
        """Run a forward pass through the VLM, returning a CausalLMOutputWithPast instance (contains loss)."""

        # Handle Inference (leverage cache, short-circuit on just LLM forward)
        if input_ids.shape[1] == 1 and past_key_values is not None: # input_ids[batch_size,length]输入文本的id， 若输入只有一个token，且有历史缓存信息，则使用缓存信息
        ...
        with torch.set_grad_enabled(self.vision_backbone_requires_grad):
            if isinstance(pixel_values, dict):
                ...
                # 将图片中属于dino和siglip的部分提取出来，并且分别交给各自的视觉处理器处理
                # view_key: 图像的类别，左边，右边...
                # view_value: {dino:..., siglip:...}   k: dino, siglip
        ...
        multimodal_embeddings = torch.cat(
            [
                input_embeddings[multimodal_indices, :1, :],  # 文本的输入的特殊标记或文本的开始部分
        ...
        if len(unimodal_indices) == 0:
            ...
            fused_embeddings = torch.vstack([multimodal_embeddings, unimodal_embeddings]) #在第一个维度上拼接
    ...
    def generate(self, image: Image, prompt_text: str, **kwargs: str) -> str:
        ...
        print(f'patch_features: {self.projected_patch_embeddings}')
